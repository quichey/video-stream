...
def extract_one_time_token(request):
    if not request.data:
        return None
    form_data = json.loads(request.data)
    return form_data.get("one_time_token")
...
def extract_user_info(request):
    ...
    form_data = json.loads(request.data)
    # TODO: change later to something like request.form['username']
    user_info = {"id": form_data.get("user_id"), "name": form_data.get("user_name")}
    return user_info
...
def has_user_info(request):
    ...
    if not user_info:
        return False
    return (
        user_info and (user_info.get("id") is not None) and (user_info.get("id") >= 0)
    )
